...
const highlightColors = {
  "reccommended-before": "bg-rose-400",
  "reccommended-after": "bg-teal-400",
...
function Courses() {
  ...
  const classes = data[dept].classes;
  ...
  const highlights = selectedID ? generateHighlighter(classes, selectedID, true) : {};
  const uniqueLevels = [...new Set(classes.map(c => c.Level.name))]
        ...
        <div className="flex flex-wrap mb-5">
          {Object.entries(highlightColors).map(([key, value]) => (
              <div className="flex items-center gap-1 mr-10">
                ...
                <div className="capitalize font-semibold">{key.replace('-', ' ')}</div>
              </div>
            ))}            
        </div>
      )}
      <div className="flex gap-6">
        {uniqueLevels.map(level => (
          <section className="min-w-60 flex-1 flex flex-col gap-5">
            <div className="w-full bg-neutral-800 text-center p-3 rounded uppercase font-bold">{level}</div>
            {classesFromLevel(level).map(c => (
              <Class 
                c={c}
                onClick={() => setSelectedID(c.id)} 
                onDoubleClick={() => setClassPopup(c)}
                highlight={highlightColors[highlights[c.id]]} // this seems silly lol
              />
            ))}
          </section>
        ))}
